...
class BACCA(nn.Module):
    def __init__(self, config, word_emb, emo_emb):
        ...
        self.word_encoder = nn.GRU(self.word_dim, 64, batch_first=True, bidirectional=True)
        self.comment_encoder = nn.GRU(self.word_dim, 64, batch_first=True, bidirectional=True)
    ...
    def forward(self, content, comments):
        ...
        xa_cache = []
        xc_cache = []
        for sentence in embedded_content:

            x1, word_lstm_weight = self.word_encoder(sentence)
        ...
        coatten = self.coattention(xa, xc)
        ...
        for sentence in embedded_content_emo:
            ...
            x3 = self.conv1(sentence)

            x3_max = self.pool(x3).squeeze(-1)
            x3_2 = self.conv2(sentence)
        ...
        for comment in embedded_comment_emo:
            ...
            x4 = self.conv1(comment)
            x4_max = self.pool(x4).squeeze(-1)
            x4_2 = self.conv2(comment)
        ...
        xb = xb.transpose(1, 0)
        xd = xd.transpose(1, 0)
        coatten_emo = self.coattention(xb, xd)
        co = torch.cat([coatten,coatten_emo],dim=1)
        preds = self.fc(co)
...
if __name__ == "__main__":
    ...
    defend = dEFENDNet(config, word_emb, emo_emb).to(device)
