...
class ExperimentRun:
    ...
    def __init__(self, config_filepath):
        ...
        vocabulary = []
        with open(vocabulary_filepath, "r+", encoding='utf-8') as f_in:
            for line in f_in:
...
def print_word_vectors(word_vectors, write_path):
    ...
    with open(write_path, "wb") as f_write:
        for key in word_vectors:
            value = " ".join(map(str, numpy.round(word_vectors[key], decimals=6)))
            vec = key + ' ' + value
            #print(vec)
            f_write.write(vec.encode())
            f_write.write('\n'.encode())
    ...
    with open(write_path, "wb") as f_write:
        for key in all_vectors:
            if key == 'the':
                ...
                print(key)
            value = " ".join(map(str, numpy.round(all_vectors[key], decimals=6)))
            vec = key + ' ' + value
            # print(vec)
            f_write.write(vec.encode())
            f_write.write('\n'.encode())
...
def load_constraints(constraints_filepath, vocabulary):
    ...
    constraints = set()
    with open(constraints_filepath, "r+", encoding='utf-8') as f:
        for line in f:
